...
def main() -> int:
    ...
    for key, node_list in key_to_nodes.items():
        ...
        if latency is None:
            ...
            continue
        g = grade(latency)
        rows.append({"node": "; ".join(node_list), "host": key[0], "port": key[1], "latency_ms": round(latency, 1), "grade": g})
        if g == "fast":
    ...
    with (QUALITY_DIR / "probe.csv").open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["node", "host", "port", "latency_ms", "grade"])
        writer.writeheader()
